...
int bind_socket(int in_server_sock, struct sockaddr_in *in_server_add) {
  int reuse = 1;
  if (setsockopt(in_server_sock, SOL_SOCKET, SO_REUSEADDR, &reuse,
                 sizeof(int)) < 0) {
    perror("Error bind_socket()");
    return -1;
  }
  int bind_result = bind(in_server_sock, (struct sockaddr *)in_server_add,
                         sizeof(*in_server_add));
  while (bind_result < 0 && errno == EADDRINUSE) {
    sleep(1);
    bind_result = bind(in_server_sock, (struct sockaddr *)in_server_add,
                       sizeof(*in_server_add));
  }
